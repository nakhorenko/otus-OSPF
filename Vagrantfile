# Define the machines with their configurations
MACHINES = {
  :router1 => {
...
Vagrant.configure("2") do |config|
  ...
  MACHINES.each do |boxname, boxconfig|
    ...
    config.vm.define boxname do |box|
      ...
      box.vm.box = config[:box_name]
      box.vm.hostname = config[:vm_name]

      if boxconfig[:vm_name] == "router3" # запускаем playbook после развертывания 3-ей виртуалки
