...
Для каждой ВМ я создал отдельный `frr.conf` (frr_r1.conf, frr_r2.conf, frr_r3.conf)
...
Отключим интерфейс enp0s9, немного подождем и снова запустим трассировку до ip-адреса 192.168.10.1
...
Для разворачивания стенда с ассиметричным роутингом меняем в начальном Playbook - `ospf.yml` value на 0
